...
class DataTrainingArguments:
    ...
    train_files: Optional[str] = field(default=None, metadata={"help": "The input training data file (a text file)."})
    validation_file: Optional[str] = field(
        default=None,
        metadata={"help": "An optional input evaluation data file to evaluate the perplexity on (a text file)."},
    )
...
def main():
    ...
    if data_args.dataset_name is not None:
        ...
        reg_key_inv={}
        reg_dict={}
        for i,val in enumerate(set(lang_data[model_args.lang_family].values())):
            reg_key[i]=val
            reg_key_inv[val]=i
        for i,val in lang_data[model_args.lang_family].items():
            reg_dict[i]=reg_key_inv[val]

        count=0

        for x in os.listdir(data_args.train_files):
            if x!='.DS_Store' and x!='readme.md' and x!='family.txt' and not x.startswith('._'):
                fname = x.split('.txt')[0]
    ...
    if data_args.line_by_line:
        ...
        for x in all_data:   
            ...
            all_tokenized_datasets[x] = tokenized_datasets
        ...
        for x in all_tokenized_datasets:
            tokenized_datasets = all_tokenized_datasets[x]
    ...
    # import pickle
    # with open('/Users/faisal/projects/run_mlm/all_train.pickle','wb') as f:
